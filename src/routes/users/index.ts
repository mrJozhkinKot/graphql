...
const plugin: FastifyPluginAsyncJsonSchemaToTs = async (
    ...
    async function (request, reply): Promise<UserEntity> {
      ...
      if (!subscriber) {
        return reply.code(404).send({message: ERROR_USER_NOT_FOUND})
         ...
         const updateUser = await fastify.db.users.change(user.id, {subscribedToUserIds: [subscriber.id, ...user.subscribedToUserIds] })
  ...
  fastify.post(
    ...
    async function (request, reply): Promise<UserEntity> {
      ...
      const updatedUser = await fastify.db.users.change(user?.id, {subscribedToUserIds: user.subscribedToUserIds.filter(subscriberId => subscriberId !== subscriber.id)})
